...
export default function AdminOrders({
  ...
  const fetchOrders = async () => {
    try {
      const { data, error } = await supabase
        .from("orders")
        .select(
          "id, order_number, customer_name, customer_phone, department, items, items_count, total_amount, status, delivery_address, delivery_lat, delivery_lng, created_at, scheduled_at",
        )
  ...
  const filtered = orderList.filter((o) => {
    const orderedItems = Array.isArray(o.items) ? o.items : [];
    const matchSearch =
    ...
    try {
      ...
      if (dispatch) {
        ...
        if (dispatch.rider_id) {
          ...
          if (riderError) {
            console.error(
              "Failed to release rider after completion:",
              riderError.message,
            );
          }
        }
  ...
  const handleStatusChange = async (id: string, newStatus: string) => {
    ...
    try {
      ...
      if (error) {
        ...
        toast.success(
          `Order status updated to "${statusLabel[newStatus] || newStatus}"`,
        );
      }
    } catch {
      toast.error("Failed to update order status");
  ...
  const handleOrderReady = async (order: Order) => {
    ...
    try {
      if (order.status !== "order_ready") {
        ...
        setOrderList((prev) =>
          prev.map((o) =>
            o.id === order.id ? { ...o, status: "order_ready" } : o,
          ),
        );
                ...
                return (
                  <React.Fragment key={order.id}>
                    ...
                    >
                      ...
                      <td className="px-5 py-3 text-sm text-foreground min-w-[240px]">
                        {orderedItems.length > 0 ? (
                          <div className="space-y-1">
                            {orderedItems.slice(0, 2).map((item, index) => (
                              <div
                            ...
                            {orderedItems.length > 2 && (
                              <p className="text-xs font-medium text-muted-foreground">
                                +{orderedItems.length - 2} more product
                                {orderedItems.length - 2 !== 1 ? "s" : ""}
                              </p>
                            )}
                            ...
                            <div>
                              ...
                              {orderedItems.length > 0 ? (
                                    ...
                                    <thead className="border-b border-border bg-muted/40">
                                      <tr>
                                        {[
                                          ...
                                          <th
                                            key={`${order.id}-item-heading-${heading}`}
                                ...
                                <div className="rounded-xl border border-dashed border-border bg-background px-4 py-6 text-center text-sm text-muted-foreground">
                                  No ordered items were recorded for this order.
                                </div>
                              )}
                            </div>
