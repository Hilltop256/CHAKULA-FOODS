...
const validSections = new Set([...navItems.map((item) => item.id), 'order-dispatch']);

type AdminPanelClientProps = {
  initialSection?: string;
  initialDispatchOrderId?: string | null;
};

export default function AdminPanelClient({
  initialSection = 'overview',
  initialDispatchOrderId = null,
}: AdminPanelClientProps) {
  const safeInitialSection = validSections.has(initialSection) ? initialSection : 'overview';
  const [activeSection, setActiveSection] = useState(safeInitialSection);
  const [sidebarCollapsed, setSidebarCollapsed] = useState(false);
  const [dispatchOrderId, setDispatchOrderId] = useState<string | null>(initialDispatchOrderId);
  const { user, profile } = useAuth();

  const adminName = profile?.full_name || user?.email?.split('@')?.[0] || 'Admin';

  const updateAdminUrl = (section: string, orderId?: string | null) => {
    if (typeof window === 'undefined') return;

    const url = new URL(window.location.href);
    url.searchParams.set('section', section);

    if (section === 'order-dispatch' && orderId) {
  ...
  const navigateToSection = (section: string) => {
    if (section !== 'order-dispatch') setDispatchOrderId(null);
  ...
  const handleNavigateToDispatch = (orderId: string) => {
    ...
    updateAdminUrl('order-dispatch', orderId);
  };

  const handleBackToOrders = () => {
    setDispatchOrderId(null);
    setActiveSection('orders');
    updateAdminUrl('orders');
  };
