...
// 1. Define the type for your database products
...
// 2. Accept the products array as a prop
export default function RestaurantPageClient({ products = [] }: { products: RestaurantItem[] }) {
  const [activeCategory, setActiveCategory] = useState('sub-all');
  const [addingId, setAddingId] = useState<string | null>(null);
  const [scheduleOpen, setScheduleOpen] = useState(false);
  
  // 3. Update the state type
  const [scheduleItem, setScheduleItem] = useState<RestaurantItem | null>(null);
  ...
  // 4. Filter against the dynamic products prop instead of the hardcoded array
  const filtered =
