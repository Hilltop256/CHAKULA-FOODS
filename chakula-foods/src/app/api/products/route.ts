...
async function POST(req: NextRequest) {
  ...
  try {
    ...
    try {
      ...
      if (msg.includes("column") && (msg.includes("availableFrom") || msg.includes("availableTo") || msg.includes("availableDays"))) {
        console.warn("Scheduling columns missing, retrying create without them");
        const { availableFrom: _af, availableTo: _at, availableDays: _ad, ...rest } = productData;
        const { prisma } = await import("@/lib/prisma");
        product = await prisma.product.create({ data: rest as never });
      } else {
        throw createErr;
      }
    }
  ...
  try {
    ...
    try {
      ...
      if (msg.includes("availableFrom") || msg.includes("availableTo") || msg.includes("availableDays")) {
        ...
        const { availableFrom: _af, availableTo: _at, availableDays: _ad, ...rest } = updateData;
        const { prisma } = await import("@/lib/prisma");
        product = await prisma.product.update({
          where: { id },
          data: rest,
    ...
    try {
      ...
      return NextResponse.json(await (await import("@/lib/prisma")).prisma.product.findUnique({ where: { id } }));
    }
  } catch (error) {
    const message = error instanceof Error ? error.message : "Unknown error";
    ...
    if (message.includes("does not exist") || message.includes("P0001") || message.includes("connection") || message.includes("prisma")) {
      try {
        const body = await req.json();
        const { id: _id, image: _img, ...restData } = body;
        if (!_id) {
          return NextResponse.json({ error: "Product ID required" }, { status: 400 });
        }
        ...
        const result = await supabaseUpdate("Product", _id, updateData);
        if (result && result.length > 0) {
